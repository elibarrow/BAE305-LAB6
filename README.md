...
As stated in the summary of the lab, we created three circuits in Lab 6, The first circuit contained an ultrasonic sensor that detected the distance to objects in front and recorded this measured distance in Arduino IDE. The second circuit of the lab included two motors and an H-Bridge motor driver and when the code was modified, allowed a direction (forward, backward, left, right) and speed value to be inputted and change these attributes in the two motors. For the third circuit of the lab, an ultrasonic sensor was added to the second circuit, and by further modifying the code from the second circuit, the motors will turn off when the ultrasonic sensor senses an object closer than ten centimeters. For the conclusions reached from the lab, we learned how to use and the applications of an ultrasonic sensor as well as how all sensors have a resolution in which small enough changes cannot be detected by the sensing system. The precision and accuracy of the sensing system should also be verified and checked due to certain applications of sensors due to small margins of error in the sensor's application. For the second circuit, we learned how to build a circuit with an H-Bridge motor drive and write code that could change the individual speed of each of the two motors as well as the direction through user input. Lastly, we learned how to create a program that, using a combination of the first and second circuits, allowed the motor to change speed and direction through user input. However, once the ultrasonic sensor detected an object closer than ten centimeters, the motors would stop preventing it from crashing into anything if it was moving.
